...
export async function POST(request: NextRequest) {
  return handleError(
    ...
    },
    {
      ...
      showToast: false, // API endpoints shouldn't show toasts
      fallbackFn: async () => {
        // Return a fallback response indicating the payment was queued
        return NextResponse.json(
          { 
            success: false, 
            error: 'Payment processing failed, but has been queued for retry',
            queued: true 
          },
          { status: 202 }
        )
